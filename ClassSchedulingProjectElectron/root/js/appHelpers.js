...
let fetchData = (e, callback) => {
    ...
    fetch(`${host}/home/fetchEvents?filterterms=${filterterms}&sessionID=${localStorage.getItem("sessionID")}`).then(response => response.json()).then((data) => {
        if (data !== "error") {
...
function fetchEventTemplates(e, callback){
    $("#s3").text("Fetching Event Templates");
    try {
        localStorage.setItem(e.id, e.value);
    } catch {
    }
    fetch(`${host}/home/fetchEventTemplates?programID=${e.value}`).then(res => res.json()).then(data => {
        console.log(data);
        if(data){
