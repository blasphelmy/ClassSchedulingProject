        ...
        fetch(`/home/fetchEventTemplates?programID=${e.value}`).then(res => res.json()).then(data => {
            ...
            if(data){
                ...
                caldata.EventTemplates.map(function(o, i){
                    let color = gColors[colorIndex++ % gColors.length];
                    o.Active = false;
...
function checkEventSanity(event){
    ...
    event.extendedProps.Year = Number(elements.year.val());
    event.extendedProps.Quarter = Number(elements.quarter.val());
...
//i need to give credits where is due more often. not intentional...just lazy..
...
const rgbToHex = (colorRGB) => "#" + ((1 << 24) + (colorRGB[0] << 16) + (colorRGB[1] << 8) + colorRGB[2]).toString(16).slice(1);
